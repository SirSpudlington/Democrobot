...
function InfoPanel(president, leadAdmin, votersCommitee, vice) {
    ...
    if (period == "Accepting Candidate Applications") {
        info.addFields(
            { name: 'Voting starts in', value: time2([time_until_new - (Times.AppicationTime + Times.VotingTime) ], "R")},
        )
    } else if (period == "Voting") {
        info.addFields(
            { name: 'Voting ends in', value: time2([time_until_new - Times.AppicationTime], "R")},
        )
    }
